...
| Proprietary versus open-weights comparison | Fixed paired conditions use `openai/gpt-5.6-terra` and the Qwen 3.5 9B-based `local/thesis-qwen3.5:9b-24k` condition with shared schema and reasoning effort | Ready in code | Confirm the exact model conditions in the approved methodology |
